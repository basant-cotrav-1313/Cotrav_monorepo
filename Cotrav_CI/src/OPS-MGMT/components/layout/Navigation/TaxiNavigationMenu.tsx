...
export type TaxiNavigationItem = {
  label: string;
  path: string;
  count: number;
...
export type TaxiNavigationGroup = {
  id: string;
  title: string;
  total: number;
  items: TaxiNavigationItem[];
};

type TaxiNavigationMenuProps = {
  groups: TaxiNavigationGroup[];
  collapsed: Record<string, boolean>;
  onToggleGroup: (groupId: string) => void;
  onNavigate: (path: string) => void;
};

const TaxiNavigationMenu: React.FC<TaxiNavigationMenuProps> = ({ groups, collapsed, onToggleGroup, onNavigate }) => {
  return (
