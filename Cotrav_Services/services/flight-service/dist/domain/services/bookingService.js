...
exports.assignFlightBooking = assignFlightBooking;
const errors_1 = require("@cotrav/errors");
const logger_1 = __importDefault(require("@cotrav/logger"));
const bookingRepository = __importStar(require("../../infrastructure/db/repositories/bookingRepository"));
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function computeTotal(dto) {
    return (dto.total_ex_tax_fees +
        dto.tax_and_fees +
        dto.gst_k3 +
...
function computeMgmtFee(booking, dto, total) {
...
function computeInvoiceTax(flightMgmtFee, billingStateId) {
...
async function processSegments(bookingId, segments, isReturn) {
...
// ─── Service ──────────────────────────────────────────────────────────────────
async function assignFlightBooking(rawBody, authId) {
    logger_1.default.info({ layer: "service", fn: "assignFlightBooking" }, "Assigning flight booking");
    const dto = booking_dto_1.AssignBookingRequest.validate(rawBody);
    // 1. Verify booking exists
    const booking = await bookingRepository.getFlightBookingById(dto.booking_id);
    if (!booking) {
        throw new errors_1.AppError("Booking not found", "APP_ERROR", 404);
    }
    ...
    const { igstRate, cgstRate, sgstRate, igst, cgst, sgst, taxCharge } = computeInvoiceTax(flightMgmtFee, billingStateId);
    ...
    logger_1.default.info({ layer: "service", fn: "assignFlightBooking", bookingId: dto.booking_id, invoiceId }, "Flight booking assigned successfully");
    return {
        success: true,
        booking_id: dto.booking_id,
