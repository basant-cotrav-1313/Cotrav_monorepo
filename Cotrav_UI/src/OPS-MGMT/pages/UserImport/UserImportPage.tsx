...
//     {visibleImportedUsers.length} records
//   </span>
// </div>
...
import OpsMainLayout from "@/OPS-MGMT/layouts/MainLayout";
import type { RealmTab } from "@/OPS-MGMT/modules/user-management/user-import/types/userImport.types";
import { 
  useUserImport, 
  useKeycloakUsers,
  useKeycloakRoles,
  ...
  ImportedUsersTable,
  ImportLoader
} from "@/OPS-MGMT/modules/user-management/user-import";

const UserImportPage: React.FC = () => {
  // ← activeRealm owned by page, shared between both hooks
  const [activeRealm, setActiveRealm] = useState<RealmTab>("ops");

  const { users, isLoadingUsers, usersError, fetchUsers } = useKeycloakUsers(activeRealm);
  const { rolesMap, isLoadingRoles } = useKeycloakRoles();
  ...
  const {
    ...
    isImporting,
    importResult,
  } = useUserImport({ activeRealm, onImportSuccess: fetchUsers });

  return (
    <OpsMainLayout 
      pageTitle="User Management" 
      pageSubtitle="Import users from legacy DB to Keycloak realms"
    >
      <div className="px-6 py-6">
        <div className="mx-auto max-w-6xl rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Switch realm and queue a user import batch.
              </p>
            </div>
            <a 
              href="/ops-mgmt/user-management-dashboard" 
          ...
          <RealmTabSwitch 
            ...
            onChange={setActiveRealm} 
          />

         <ImportButton onClick={triggerImport} isLoading={isImporting} />
          ...
          <div className="grid gap-4 lg:grid-cols-[2fr_1fr]">
            ...
            <ImportSummary
              activeRealm={activeRealm}
              identifierType={identifierType}
              recordCount={normalizedIdentifiers.length}
              dryRun={dryRun}
          ...
          {/* <ImportedUsersTable
            activeRealm={activeRealm}
            users={visibleImportedUsers}
            onRealmChange={setActiveRealm}
          /> */}

          <ImportedUsersTable
  activeRealm={activeRealm}
  users={users}              // ← was visibleImportedUsers
  isLoading={isLoadingUsers}
  error={usersError}
  onRealmChange={setActiveRealm}
  rolesMap={rolesMap}           // ← pass
  isLoadingRoles={isLoadingRoles} // ← pass
/>

        </div>
      </div>
      <ImportLoader isImporting={isImporting} />
    </OpsMainLayout>
